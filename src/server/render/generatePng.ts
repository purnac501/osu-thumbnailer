...
export async function generatePng(options: GenerateOptions): Promise<Buffer> {
    ...
    try {
        ...
        await page
            ...
            .then(() => page.waitForFunction(() => window.__THUMBNAIL_READY === true, null, { timeout: 30000 }));
        ...
        if (meta.width !== options.width || meta.height !== options.height) {
            throw new Error(`Rendered PNG is ${meta.width}x${meta.height}, expected ${options.width}x${options.height}`);
        }
        return png;
    }
    finally {
