...
export function detectStatus(score: ApiScore, beatmapMaxCombo?: number): PlayStatus {
  ...
  // When is_perfect_combo is false with 0 misses:
  // Check if score.max_combo is close to the beatmap max combo (dropped slider ends)
  // or significantly lower (slider break in the middle of the map).
  ...
  if (mapMaxCombo && scoreMaxCombo !== undefined) {
    const maxDroppedEnds = Math.min(
      Math.max(15, score.beatmap?.count_sliders ?? 20),
      Math.max(2, Math.floor(mapMaxCombo * 0.08)),
    );
