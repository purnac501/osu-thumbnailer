...
/**
 * Dedicated render route used by PNG generation and the compare script.
 * URL params:
 *   /render?url=<osu score url>&template=reference&scale=1&edits=<json>
 *   /render?fixture=reference&template=reference&scale=0.8
 */
    ...
    const load = async () => {
      try {
        if (fixture) {
          ...
          const json = (await res.json()) as { data: ThumbnailData };
          setData(json.data);
        } else if (url) {
