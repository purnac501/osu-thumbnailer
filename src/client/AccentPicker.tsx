...
export function ColorPicker({ color, onChange, label = "Color", align = "left", }: {
    color: string;
    ...
    return (<Popover.Root>
      ...
      <Popover.Content className="accent-popover" size="2" width="220px" align={align === "right" ? "end" : "start"} sideOffset={6}>
          ...
          <TextField.Root className="accent-hex-field" size="2" aria-label={`${label} hex color`} value={hexDraft} maxLength={6} onChange={(event) => {
