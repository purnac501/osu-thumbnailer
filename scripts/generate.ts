...
/**
 * Headless PNG generation without the browser UI:
 *   npm run generate -- "https://osu.ppy.sh/scores/osu/123" \
 *     --template reference --resolution 2560x1440
 */
async function main() {
  ...
  try {
    ...
    if (!args.out) fs.writeFileSync(outFile, png);
